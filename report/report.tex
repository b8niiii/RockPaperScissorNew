...
% -----------------------------
\subsection{Model 1: Simple CNN (Ray Tune)}

\paragraph{Best hyperparameters (Ray Tune).}
...
\paragraph{Confusion matrix (test set).}
Figure~\ref{fig:model1-cm} reports the confusion matrix on the test set (rows = true class, columns = predicted class), with the corresponding raw counts shown in Table~\ref{tab:model1-cm-counts}. % [file:60]

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.62\textwidth]{model1_confusion_matrix.png}
  \caption{Model 1: confusion matrix on the test set.}
  \label{fig:model1-cm}
\end{figure}
...
Overall, this simple CNN is computationally efficient and already generalizes well, making it a solid reference point for the upcoming comparison with deeper architectures and cross-validation.
...
% -----------------------------
\subsection{Results for Model 2: Medium CNN (K-fold Cross-Validation)}

\paragraph{Training configuration.}
Unlike Model~1, Model~2 was not re-tuned: we fixed the training hyperparameters to the previously selected learning rate and batch size, and evaluated the model via K-fold cross-validation. % [file:92]
The model was optimized with SGD (momentum $=0.9$), using $lr = 5.115859\times 10^{-4}$, $batch\_size = 16$, and trained for $20$ epochs per fold. % [file:92]
Cross-validation was performed with $K=10$ folds (shuffle enabled, random\_state $=42$), and the same preprocessing/augmentation pipeline used in the earlier experiment was retained. % [file:92]
...
% -----------------------------
\subsection{Model 3: Residual CNN (K-fold Cross-Validation)}

\paragraph{Training configuration.}
Model~3 was evaluated using K-fold cross-validation (K = 5, shuffle enabled, random\_state = 42), keeping the same preprocessing/augmentation pipeline adopted in the previous experiments. % [file:95]
Training used CrossEntropyLoss and SGD with momentum ($0.9$), with $lr = 5.115859\times 10^{-4}$ and $batch\_size = 16$, for $20$ epochs per fold. % [file:95]
